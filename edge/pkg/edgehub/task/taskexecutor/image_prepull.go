...
func pullImages(taskReq types.NodeTaskRequest) fsm.Event {
	...
	go func() {
		...
		event.ExternalMessage = imageStatus
		edgeutil.ReportTaskResult(edgeCoreConfig, taskReq.Type, taskReq.TaskID, event)
	}()
	return fsm.Event{}
}
...
func sendResponseMessage(nodeName, jobName, pullMsg string, imageStatus []v1alpha1.ImageStatus, state string) {
